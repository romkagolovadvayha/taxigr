...
export function vkCallbackHtml(success: boolean): string {
  ...
  const message = success
    ? 'Вернитесь в приложение «Такси Грахово» — вход завершится автоматически.'
    : 'Закройте это окно и попробуйте войти через VK ещё раз.';
  const community = success
    ? '<p><a href="https://vk.ru/taxigr">Написать сообществу и включить уведомления</a></p>'
    : '';
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>body{margin:0;background:#f4f4f2;color:#181818;font-family:Arial,sans-serif;display:grid;min-height:100vh;place-items:center}.card{max-width:520px;margin:24px;padding:32px;border:1px solid #ddd;border-radius:28px;background:white;text-align:center}h1{margin:0 0 16px}p{line-height:1.5}.mark{width:64px;height:64px;border-radius:18px;background:#0077ff;color:white;display:grid;place-items:center;margin:0 auto 20px;font-size:30px;font-weight:800}a{display:inline-block;padding:14px 18px;border-radius:14px;background:#0077ff;color:white;text-decoration:none;font-weight:700}</style></head><body><main class="card"><div class="mark">VK</div><h1>${title}</h1><p>${message}</p>${community}</main></body></html>`;
}
