...
describe("Telegram update processing", () => {
  ...
  it.each([
    ...
    "handles a shared contact with expected phone %s",
    async (expectedPhone, matches) => {
      mocks.telegramStartPayload.mockReturnValue(null);
      mocks.extractOwnTelegramPhone.mockReturnValue("+79990000000");
      mocks.firstRow.mockResolvedValue({
        id: "challenge-id",
      ...
      await processTelegramUpdate(
        telegramUpdateSchema.parse({
          message: {
            from: { id: 42 },
            chat: { id: 42, type: "private" },
            contact: { phone_number: "+79990000000", user_id: 42 },
          },
        }),
      );
      expect(mocks.execute).toHaveBeenCalledWith(
        expect.stringContaining("UPDATE telegram_auth_challenges"),
        [
          "+79990000000",
          matches ? null : "PHONE_MISMATCH",
          matches,
          "challenge-id",
        ],
      );
      expect(mocks.sendTelegramConfirmation).toHaveBeenCalledWith(
        "42",
        matches,
      );
    },
  ...
  it("does not authenticate a contact that is not owned by the sender", async () => {
    mocks.telegramStartPayload.mockReturnValue(null);
    mocks.extractOwnTelegramPhone.mockReturnValue(null);
    await processTelegramUpdate(
      telegramUpdateSchema.parse({
        message: {
          from: { id: 42 },
          chat: { id: 42, type: "private" },
          contact: { phone_number: "+79990000000", user_id: 999 },
        },
      }),
    );
