...
- [ ] Проверить webhooks MAX и Telegram, включая секретные заголовки, передачу собственного номера и callback-события (`message_callback` в MAX, `callback_query` в Telegram).
...
- [ ] Весь флоу заказа в Telegram и MAX: принятие, выезд, прибытие, старт, ожидание, завершение, отмена, повышение цены и оценка.
- [ ] Проверить, что свободным водителям не виден номер пассажира, а после принятия оба участника видят ФИО, телефоны и нативные геоточки.
